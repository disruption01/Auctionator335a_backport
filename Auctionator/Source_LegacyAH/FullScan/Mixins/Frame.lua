...
function AuctionatorFullScanFrameMixin:CacheScanData()
  ...
  self:ResetData()
  local auctionCount = GetNumAuctionItems("list") or 0
  self.waitingForData = auctionCount
  ...
  -- WoW 3.3.5a auction rows are indexed 1..N. The upstream Classic code
  -- starts this batching loop at 0, which feeds nil into GetItemInfoInstant.
  -- Keep ProcessBatch's limit exclusive by passing N + 1.
  self:ProcessBatch(
    1,
    150,
    auctionCount + 1
...
function AuctionatorFullScanFrameMixin:ProcessBatch(startIndex, stepSize, limit)
  ...
  Auctionator.EventBus:Fire(self,
    ...
    math.floor((0.2 + ((startIndex - 1) / math.max(1, limit - 1)) * 0.8) * 100) / 100
  )
  ...
  while i < startIndex+stepSize and i < limit do
    ...
    info[Auctionator.Constants.AuctionItemInfo.ItemID] = itemID
        ...
        return function()
          ...
          Auctionator.Utilities.DBKeyFromLink(link, function(dbKeys)
            ...
            table.insert(self.scanData, {
              auctionInfo = (function() local x={GetAuctionItemInfo("list", index)}; x[Auctionator.Constants.AuctionItemInfo.ItemID]=C_Item.GetItemInfoInstant(link); return x end)(),
              itemLink      = link,
            })
...
local function GetInfo(auctionInfo)
  local available = auctionInfo[Auctionator.Constants.AuctionItemInfo.Quantity] or 0
  local buyoutPrice = auctionInfo[Auctionator.Constants.AuctionItemInfo.Buyout] or 0
  local effectivePrice = available > 0 and (buyoutPrice / available) or 0
