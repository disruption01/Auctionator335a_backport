...
-- 3.3.5a exposes the item texture directly as the second return value of
-- GetAuctionItemInfo().  This is available even for items whose full
-- GetItemInfo cache entry has not arrived yet, so keep it as an immediate icon
-- source for the Shopping result table.
local function GetIconTexture(entries)
  for _, entry in ipairs(entries or {}) do
    if entry.iconTexture then
      return entry.iconTexture
    end
    if entry.info and entry.info[2] then
      return entry.info[2]
    end
    if entry.itemLink and GetItemIcon then
...
function AuctionatorDirectSearchProviderMixin:AddFinalResults()
  ...
  for key, entries in pairs(self.resultsByKey) do
    ...
    local possibleResult = {
      ...
      iconTexture = GetIconTexture(entries),
    }
    local item = Item:CreateFromItemID(C_Item.GetItemInfoInstant(key))
    item:ContinueOnItemLoad(function()
      -- At this point the 3.3.5 item cache is ready. Resolve the icon again
      -- from the native item object/GetItemInfo instead of relying only on the
      -- auction row texture captured before the cache finished loading.
      local loadedIcon = item.GetItemIcon and item:GetItemIcon() or nil
      if not loadedIcon then
        loadedIcon = select(10, GetItemInfo(key))
      end
      if loadedIcon then
        possibleResult.iconTexture = loadedIcon
      end
