...
function AuctionatorAHThrottlingFrameMixin:OnEvent(eventName, ...)
  ...
  elseif eventName == "CHAT_MSG_SYSTEM" then
    local msg = ...
    -- Use the stock message when available. Private 3.3.5a cores frequently
    -- alter/localise this text, so AUCTION_OWNED_LIST_UPDATE below is the
    -- authoritative completion signal for cancellations.
  ...
  elseif eventName == "AUCTION_OWNED_LIST_UPDATE" then
    if self.waitingForStatusMessage then
      self:ResetTimeout()
      FrameUtil.UnregisterFrameForEvents(self, AUCTIONS_UPDATED_EVENTS)
      self.waitingForStatusMessage = false
    end
