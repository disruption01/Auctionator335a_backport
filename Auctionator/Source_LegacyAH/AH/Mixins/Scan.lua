...
local function ParamsForBlizzardAPI(query, page)
  -- Stock 3.3.5a signature:
  --   QueryAuctionItems(name, minLevel, maxLevel, invTypeIndex, classIndex,
  --     subClassIndex, page, isUsable, qualityIndex, getAll)
  --
  -- Auctionator's advanced-search category tree stores these legacy indices on
  -- its single selected filter. Exact matching is handled by Auctionator after
  -- the page is returned because this client has no exactMatch parameter.
  local filter = query.itemClassFilters and query.itemClassFilters[1] or nil
  local invTypeIndex = filter and filter.inventoryTypeIndex or nil
  local classIndex = filter and filter.classIndex or nil
  local subClassIndex = filter and filter.subClassIndex or nil

  return query.searchString, query.minLevel, query.maxLevel, invTypeIndex,
    classIndex, subClassIndex, page, nil, query.quality, false
end
