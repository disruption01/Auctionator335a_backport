...
function AuctionatorBuyCurrentPricesFrameMixin:Init()
  ...
  -- TBC Classic reserves a thin strip at the bottom of the price table for
  -- "Load higher prices" / "Stop loading". The plain 3.3.5a ScrollBox has no
  -- clipping mask, so without this the last full row visually sits under that
  -- control. Shorten only the internal scroll viewport; keep the outer table
  -- and footer buttons in their existing positions.
  local scrollArea = self.SearchResultsListing and self.SearchResultsListing.ScrollArea
  local header = self.SearchResultsListing and self.SearchResultsListing.HeaderContainer
  if scrollArea and header then
    scrollArea:ClearAllPoints()
    scrollArea:SetPoint("TOPLEFT", header, "BOTTOMLEFT", 15, -3)
    scrollArea:SetPoint("BOTTOMRIGHT", self.SearchResultsListing, "BOTTOMRIGHT", -3, 24)
  end
