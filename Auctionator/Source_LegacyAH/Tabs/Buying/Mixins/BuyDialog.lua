...
local function ResolveBuyDialogFontStrings(self)
  -- On 3.3.5a the Layer regions from an inherited virtual template can be
  -- rendered without getting their $parent global names. In that case the
  -- old fallback created a second set of FontStrings, leaving the original
  -- placeholder text visible underneath and producing the overlapped dialog.
  -- Reuse the inherited regions first, in the same order as BuyDialog.xml.
  local fontStrings = {}
  ...
  self.PurchaseDetails = self.PurchaseDetails or fontStrings[1]
  self.UnitPrice = self.UnitPrice or fontStrings[2]
  self.NumberPurchased = self.NumberPurchased or fontStrings[3]

  if self.PurchaseDetails == nil then
    self.PurchaseDetails = self:CreateFontString(nil, "OVERLAY", "GameFontHighlightLarge")
  end
  if self.UnitPrice == nil then
    self.UnitPrice = self:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
  end
  if self.NumberPurchased == nil then
    self.NumberPurchased = self:CreateFontString(nil, "OVERLAY", "GameFontNormal")
  end

  -- Force the intended 400x140 dialog layout. Explicit widths are important
  -- on the old client, otherwise formatted money strings can wrap oddly.
  self.PurchaseDetails:ClearAllPoints()
  self.PurchaseDetails:SetPoint("CENTER", self, "CENTER", 0, 30)
  self.PurchaseDetails:SetWidth(350)
  self.PurchaseDetails:SetJustifyH("CENTER")

  self.UnitPrice:ClearAllPoints()
  self.UnitPrice:SetPoint("TOP", self.PurchaseDetails, "BOTTOM", 0, -6)
  self.UnitPrice:SetWidth(350)
  self.UnitPrice:SetJustifyH("CENTER")

  self.NumberPurchased:ClearAllPoints()
  self.NumberPurchased:SetPoint("TOP", self.UnitPrice, "BOTTOM", 0, -8)
  self.NumberPurchased:SetWidth(350)
  self.NumberPurchased:SetJustifyH("CENTER")
end

function AuctionatorBuyDialogMixin:OnLoad()
  self:RegisterForDrag("LeftButton")

  ResolveBuyDialogFontStrings(self)

  if self.WarningDialog and not self.WarningDialog.Text then
    local warningRegions = { self.WarningDialog:GetRegions() }
    for _, region in ipairs(warningRegions) do
      if region and region.GetObjectType then
        local ok, objectType = pcall(region.GetObjectType, region)
        if ok and objectType == "FontString" then
          self.WarningDialog.Text = region
          break
        end
      end
    end
  ...
  -- Keep the hidden dialog empty until SetDetails() supplies real data.
  self.NumberPurchased:SetText("")
...
function AuctionatorBuyDialogMixin:OnHide()
  ...
  if (tonumber(self.quantityPurchased) or 0) > 0 and self.auctionData ~= nil then
