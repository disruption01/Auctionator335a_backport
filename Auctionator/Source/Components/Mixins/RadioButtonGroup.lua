...
function AuctionatorConfigRadioButtonGroupMixin:InitializeRadioButtonGroup()
  ...
  if self.groupHeadingText ~= nil and self.GroupHeading then
    self.GroupHeading.subHeadingText = self.groupHeadingText
    if self.GroupHeading.InitializeSubHeading then
...
function AuctionatorConfigRadioButtonGroupMixin:SetupRadioButtons()
  ...
  for _, child in ipairs(children) do
    if child.isAuctionatorRadio then
      ...
      if child.RadioButton and child.RadioButton.Label then
        child.RadioButton.Label:ClearAllPoints()
        child.RadioButton.Label:SetPoint("LEFT", child.RadioButton, "RIGHT", 4, 0)
        if child.labelText ~= nil then child.RadioButton.Label:SetText(child.labelText) end
      end
