...
function AuctionatorGroupsViewMixin:EnsurePools()
  if not self.ScrollBox or not self.ScrollBox.ItemListingFrame then
    return false
  end

  if not self.itemTemplate then
    self.itemTemplate = "AuctionatorGroupsViewItemTemplate"
  end
  if not self.groupTemplate then
    self.groupTemplate = "AuctionatorGroupsViewGroupTemplate"
  end

  if not self.buttonPool or self.__buttonPoolTemplate ~= self.itemTemplate then
    if self.buttonPool and self.buttonPool.ReleaseAll then self.buttonPool:ReleaseAll() end
    self.buttonPool = CreateFramePool("Button", self.ScrollBox.ItemListingFrame, self.itemTemplate)
    self.__buttonPoolTemplate = self.itemTemplate
  end
  if not self.groupPool or self.__groupPoolTemplate ~= self.groupTemplate then
    if self.groupPool and self.groupPool.ReleaseAll then self.groupPool:ReleaseAll() end
    ...
    self.__groupPoolTemplate = self.groupTemplate
  end
  return true
end
...
function AuctionatorGroupsViewMixin:Update(cache)
  ...
  if cache == nil or cache.GetAllContents == nil then
    cache = AuctionatorBagCacheFrame
  end
  self.rawItems = (cache and cache.GetAllContents and cache:GetAllContents()) or {}
...
function AuctionatorGroupsViewMixin:UpdateFromExisting()
  -- Child OnLoad scripts can replace inherited OnLoad on 3.3.5a. Make this
  -- path self-healing instead of assuming OnLoad initialized every field.
  self.collapsing = self.collapsing or {}
  self.groups = self.groups or {}
  self.rawItems = self.rawItems or {}
  self.hiddenItems = self.hiddenItems or {}
  if self.originalOpen == nil then self.originalOpen = true end
  if not self:EnsurePools() then
    return
  end
