...
function AuctionatorShoppingTabFrameMixin:OnLoad()
  ...
  -- 3.3.5a: relativeKey resolution is incomplete. Keep Shopping as the
  -- original two-pane UI: list/recents left, result table right.
  if self.ListsContainer and self.ResultsListing then
    -- Match the compact TBC Classic proportions. The left pane itself was
    -- already close; the apparent giant rows came from SetPanExtent()
    -- accidentally changing the row extent in the 3.3.5 ScrollBox shim.
    self.ListsContainer:SetWidth(280)
    if self.RecentsContainer then self.RecentsContainer:SetWidth(280) end
    self.ResultsListing:ClearAllPoints()
    self.ResultsListing:SetPoint("TOPLEFT", self.ListsContainer, "TOPRIGHT", 22, 27)
    self.ResultsListing:SetPoint("BOTTOMRIGHT", self, "BOTTOMRIGHT", -20, 0)
  end

  -- TBC Classic gives Search Options a little more breathing room than the
  -- dynamic 3.3.5 button template does. Keep this deterministic so the label
  -- never looks cramped after a search/cancel text resize.
  if self.SearchOptions and self.SearchOptions.MoreButton then
    self.SearchOptions.MoreButton:SetWidth(125)
  end

  -- Keep New List / Import / Export visually inside the left shopping-list
  -- pane. The converted relative anchors pushed Export into the right result
  -- pane on 3.3.5a. Match the TBC Classic footer instead.
  if self.ListsContainer then
    if self.NewListButton then
      self.NewListButton:ClearAllPoints()
      self.NewListButton:SetPoint("BOTTOMLEFT", self.ListsContainer, "BOTTOMLEFT", 5, -25)
    end
    if self.ExportButton then
      self.ExportButton:ClearAllPoints()
      self.ExportButton:SetPoint("BOTTOMRIGHT", self.ListsContainer, "BOTTOMRIGHT", -3, -25)
    end
    if self.ImportButton and self.ExportButton then
      self.ImportButton:ClearAllPoints()
      self.ImportButton:SetPoint("RIGHT", self.ExportButton, "LEFT", -4, 0)
    end
  end
  if self.ShoppingResultsInset and self.ResultsListing then
    self.ShoppingResultsInset:ClearAllPoints()
    self.ShoppingResultsInset:SetPoint("TOPLEFT", self.ResultsListing, "TOPLEFT", -10, -25)
    self.ShoppingResultsInset:SetPoint("BOTTOMRIGHT", self.ResultsListing, "BOTTOMRIGHT", 0, 2)
  end
  if self.ExportCSV and self.ResultsListing then
    self.ExportCSV:ClearAllPoints()
    self.ExportCSV:SetPoint("TOP", self.ResultsListing, "BOTTOM", 0, 0)
    self.ExportCSV:SetPoint("RIGHT", self, "RIGHT", -8, 10)
  end
  ...
  if Auctionator.Constants.IsLegacyAH or Auctionator.Constants.IsVanilla then
...
function AuctionatorShoppingTabFrameMixin:OnShow()
  -- 3.3.5a: do not auto-focus the search EditBox when opening the tab.
  -- Old FrameXML keeps keyboard focus here and swallows movement/Enter.
  if self.SearchOptions and self.SearchOptions.SearchString and self.SearchOptions.SearchString.ClearFocus then
    self.SearchOptions.SearchString:ClearFocus()
  end
