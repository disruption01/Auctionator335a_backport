...
local function ImportFromConnectedRealm(rootRealm)
  -- Connected realms are a later-client concept. Some 3.3.5a/private-server
  -- clients expose no GetAutoCompleteRealms at all, while others expose a stub
  -- with incompatible behaviour. Treat it as optional migration metadata.
  local connections = {}
  if type(GetAutoCompleteRealms) == "function" then
    local ok, result = pcall(GetAutoCompleteRealms)
