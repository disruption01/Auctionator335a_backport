...
function AuctionatorRetailImportLargeMoneyInputBoxMixin:OnLoad()
	...
	-- Layer regions in inherited virtual templates are not reliable on 3.3.5a.
	if not self.Icon then
		self.Icon = self:CreateTexture(nil, "ARTWORK");
		self.Icon:SetWidth(12);
		self.Icon:SetHeight(14);
		self.Icon:SetPoint("RIGHT", self, "RIGHT", -10, 2);
	end
	
	if self.iconAtlas then
		self.Icon:SetTexture([[Interface\MoneyFrame\UI-MoneyIcons]]);
		if self.iconAtlas == "auctionhouse-icon-coin-gold" then
			self.Icon:SetTexCoord(0, 0.25, 0, 1);
		elseif self.iconAtlas == "auctionhouse-icon-coin-silver" then
			self.Icon:SetTexCoord(0.25, 0.5, 0, 1);
		elseif self.iconAtlas == "auctionhouse-icon-coin-copper" then
