...
function AuctionatorRetailImportMoneyDenominationDisplayMixin:OnLoad()
	...
	-- WoW 3.3.5a can instantiate the inherited frame while dropping its
	-- Layer regions. Recreate the icon/text here so table money cells are safe.
	...
	if not self.Text then
		...
		self.Text:SetPoint("RIGHT", self, "RIGHT", -13, 0);
		if self.Text.SetShadowOffset then self.Text:SetShadowOffset(1, -1) end
		if self.Text.SetShadowColor then self.Text:SetShadowColor(0, 0, 0) end
	end

	-- 3.3.5a does not reliably apply modern XML KeyValues before OnLoad.
	-- Infer the denomination from the legacy child frame name when needed.
	if self.displayType == nil and self.GetName then
		local name = self:GetName() or "";
		if string.find(name, "CopperDisplay$") then
			self.displayType = AuctionatorRetailImportMoneyDenominationDisplayType.Copper;
		elseif string.find(name, "SilverDisplay$") then
			self.displayType = AuctionatorRetailImportMoneyDenominationDisplayType.Silver;
		elseif string.find(name, "GoldDisplay$") then
			self.displayType = AuctionatorRetailImportMoneyDenominationDisplayType.Gold;
		end
	end

	-- A direct instance of the generic denomination template can still exist
	-- while TableBuilder is constructing cells.  Do not kill the whole row;
	-- use copper as a harmless legacy fallback until SetDisplayType() is called.
...
local function Auctionator335_LegacyCoinCoords(atlas)
...
function AuctionatorRetailImportMoneyDenominationDisplayMixin:UpdateDisplayType()
	...
	-- Even when a compatibility SetAtlas exists, 3.3.5a has no Auction House
	-- coin atlases. Use the native Wrath money icon sheet explicitly.
	if textureType == TextureType.Atlas then
		self.Icon:SetTexture([[Interface\MoneyFrame\UI-MoneyIcons]]);
		l, r, b, t = Auctionator335_LegacyCoinCoords(fileOrAtlas);
